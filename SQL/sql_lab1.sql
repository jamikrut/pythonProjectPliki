...
-- 7. Wyświetl kolumny BusinessEntityID, HireDate i JobTitle dla wszystkich pracowników posiadających JobTitle „Sales Representative”, uporządkowany według BusinessEntityID, a następnie HireDate [HumanResources.Employee]
SELECT BusinessEntityID, HireDate, JobTitle
FROM HumanResources.Employee
WHERE JobTitle = 'Sales Representative'
ORDER BY BusinessEntityID, HireDate

-- 8. Wyświetl kolumny Name i ListPrice dla wszystkich produktów, które mają a ListPrice między 500 a 1000, uporządkowane według ListPrice [Production.Product]
SELECT Name, ListPrice
FROM Production.Product
WHERE ListPrice BETWEEN 500 AND 1000
ORDER BY ListPrice

-- 9. Wyświetl kolumny Name, Color i Size dla wszystkich produktów które mają kolor „Black” i rozmiar „M”, uporządkowane według Name [Production.Product]
...
-- 10. Wyświetl kolumny BusinessEntityID i HireDate dla wszystkich pracowników zatrudnionych w 2010 r., uporządkowane według HireDate [HumanResources.Employee]
SELECT BusinessEntityID, HireDate
FROM HumanResources.Employee
WHERE HireDate LIKE '2010%'
ORDER BY HireDate

-- 11. Wybierz wszystkich klientów z Niemiec i posortuj ich według PersonID. (2 zapytania) [Sales.Customer]
SELECT * -- -> Dane klientów z Niemiec
FROM Sales.Customer
WHERE TerritoryID IN (SELECT TerritoryID
                      FROM Sales.SalesTerritory
                      WHERE CountryRegionCode = 'DE') -- -> TerritoryID = 8 dla DE
ORDER BY PersonID
...
SELECT *
FROM Sales.SalesOrderHeader
WHERE ShipMethodID IN (SELECT ShipMethodID
                       FROM Purchasing.ShipMethod
                       WHERE Name = 'OVERSEAS - DELUXE'
                          OR Name = 'CARGO TRANSPORT 5') -- -> metody dostawy "OVERSEAS - DELUXE" lub "CARGO TRANSPORT 5"
ORDER BY ShipDate
